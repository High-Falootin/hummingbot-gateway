...
### Wallet File Format
Wallets are stored as JSON files at `conf/wallets/{chain}/{address}.json` with fields: `{ address, encryptedPrivateKey, network }`. The `network` field disambiguates wallets on multi-network chains (e.g., `polygon` vs `mainnet` for `chain: ethereum`).

### chainNetwork Parsing in Routes
All connector and chain routes accept both `chain` and `network` fields. The `chain` identifies the base blockchain (`ethereum`, `solana`) and `network` identifies the specific deployment (`mainnet`, `polygon`, `mainnet-beta`). Always extract both from the request body and pass to `getInstance(network)`.
...
- Chain-specific fields (e.g., `tickSpacing` for Uniswap V3, `feePct` for AMM pools) should be added alongside the standard fields, not in place of them
...
- **PancakeSwap** (Ethereum/BSC/EVM): V2 AMM, V3 CLMM, and V3 Smart Order Router swaps
- **QuickSwap** (Polygon): V2 AMM, V3 CLMM, and Router swaps (Uniswap V3 fork)
