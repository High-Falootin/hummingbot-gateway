...
import { pancakeswapInfinityRoutes } from './infinity-routes';
...
// Infinity (V4-style singleton PoolManager) routes
const pancakeswapInfinityRoutesWrapper: FastifyPluginAsync = async (fastify) => {
  await fastify.register(sensible);
  ...
  await fastify.register(async (instance) => {
    ...
    await instance.register(pancakeswapInfinityRoutes);
  });
};
...
export const pancakeswapRoutes = {
  ...
  infinity: pancakeswapInfinityRoutesWrapper,
};
